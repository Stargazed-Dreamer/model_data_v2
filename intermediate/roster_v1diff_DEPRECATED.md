> ## ⛔ 本文件已弃用（2026-08-29 标注）—— 请勿用作采集名册
>
> **文件名曾叫 `roster.md` / `roster.jsonl`，极易被误读为「702 条采集名册」。它不是。**
>
> | | 本文件 | 真正的采集名册 |
> |---|---|---|
> | 路径 | `intermediate/roster_v1diff_DEPRECATED.*` | `docs/batch_claim_ledger.jsonl`（取 `models[].model_id` 去重）|
> | 条数 | 506 | **702** |
> | 性质 | 阶段 0 的 **v1 差异清单**（`in_v1` 333 / `out_of_scope` 147 / `to_add` 26）| M 型扩容后的**批次认领表** |
> | 是否随扩容更新 | ❌ 从未更新 | ✅ 305 批全 submitted |
>
> **下文第 6 行「花名册是 model_id 的唯一权威」这句已经作废。** 702 条里有 **444 条不在本文件的任何状态中**。
> 若按本文件去核对采集覆盖率，会得出完全错误的结论
> ——实测：用它比对「骨架残留」得到 80 条（错），用 ledger 得到 26 条（对）。
>
> 本文件仅作阶段 0 的历史留档保留。生成脚本 `scripts/gen_roster.py` 与基线 `model_data_v1_clean.jsonl` 均已删除/不存在。
>
> ---
