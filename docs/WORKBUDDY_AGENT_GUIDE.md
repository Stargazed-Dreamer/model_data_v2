...
> **⚠ 文中 `temp/*.py` 指针不入库**：`.gitignore` 第 6 行排除了 `/temp/`，那些脚本是 D2–D18c 各轮整改的**本机一次性产物**，
...
**仍开着的遗留**：① 表外 **4 条**（上表那两条争议，须回原始来源定夺 → 属重采或另请拍板）；② 丙档 **85 条**（两栏矛盾，属重采），其中 **83 条**同时落在 §27 第一类宽口径 438 的重叠里；③ **406 条冗余前缀类**（如 `T0 官方一手技术报告（自报）`）仍**无任何门禁信号**，规则 6.3 刻意不覆盖；④ 来源类型缺失涨到 **1290 / 1300**，门禁放过空值 → 这批对门禁完全不可见，只能靠本清单追踪。
