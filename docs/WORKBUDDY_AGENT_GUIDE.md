...
> **⚠ 文中 `temp/*.py` 指针不入库**：`.gitignore` 第 6 行排除了 `/temp/`，那些脚本是 D2–D20 各轮整改的**本机一次性产物**，
...
- 其余登记 D18：yandex 两条 notes 三栏整段复制；~~**436 条 `self_reported` 的 `source_type` 不含「自报」**（门禁 WARN，抄写源是文档下拉表，见 §26.5 第 3 项）~~ **→ D18 已结案，见 §27**（308 条挂后缀归一、128 条空值门禁放过并改登记为待重采）；`docs/archive/交接_2026-09-01_D16结案.md` 把远端仓库写成「GitHub 私有」，实为**公共**仓库（已改正）——写交接文档时对「远端可见性」这类安全相关事实必须实测，不能凭印象。
...
**文档同日改**：`prompt.md` 两处 —— 「别把可信度值当来源类型写」那段把 607 的分布划掉、补 D18c 后的 **89 条 / 12 条记录**实测分布与剩 89 条构成，指南指针扩到 §27.1 / §27.2；**可信度等级那一行下新增显式禁令**「这七个值是穷举，写枚举外值门禁直接判 ERROR」，并把表外 4 条的两处争议原文抄进去当反例（新采集只准写这七个值，「第三方」「转述」这类限定写进 `source_type` / `notes`）。`archive/qa_report.md` 抬头十五→**十七处**、第 16 项的乙档遗留划掉指向新增的**第 17 项**、§1 与 §6.2 的 WARN 演进行同步到 122、抬头 `temp/*.py` 声明的轮次范围 `D2–D15` → `D2–D18c`（第 16/17 项已引用 D18b/D18c 的本机脚本）。`docs/archive/交接_2026-09-01_D16结案.md` 横幅补 D18c 与新基线。记忆两处同步（`docs/memory/project/project-stage3-gaps.md` 为权威副本 + 平台目录，md5 逐字比对）。
...
**文档同日改**：`prompt.md` 两处 —— 把「另有 406 条…」划掉换成 D18d 结案实测（405 / 55 / 5 / 1 与零损失论证），并**新增面向新采集的显式禁令**「不得把等级值当前缀写进 `source_type`」，同时如实写明「目前没有任何门禁规则照这类，只有本文档在守」；含自报写法种类 178 → 171 就地更正，并补上「种类数会降但含自报**条目数**不该动（2567 → 2567）」这条验收口径。`archive/qa_report.md` 抬头十七→**十八处**、新增**第 18 项**、WARN 演进行补 D18d（122 不变，改的是数据不是门禁）。`docs/archive/交接_2026-09-01_D16结案.md` 横幅补 D18d 与新 md5。记忆两处同步。
...
**文档同日改**：`prompt.md` 一处 —— 把 D18d 写下的「目前没有任何门禁规则照这类，这条禁令只有本文档在守」划掉换成 D18e 实况（规则 6.4 会报、现库命中 0、与 6.3 互斥、刻意照不到表外那条），指针扩到 **§27.4**；本指南 §27.3 遗留 ② 加删除线指向本节、抬头 `temp/*.py` 轮次范围 `D2–D18d` → `D2–D18e`；`archive/qa_report.md` 抬头十八→**十九处**、新增**第 19 项**；`docs/archive/交接_2026-09-01_D16结案.md` 横幅补 D18e 与门禁脚本 md5 变化。记忆两处同步。
...
**仍开着的遗留**：① ~~**【下一项，证据已备齐】等级值当「括号后缀」写在来源类型栏 21 条 / 3 条记录**~~ **→ ✅ D20 已结案（见 §27.6）**：第 28 轮拍板「剥等级、留限定词」，21 条同日剥净；并补了 WARN 级**规则 6.5**（现库命中确为 0，十个探针验过它真的会响）。（`官方模型卡自报（T0 直采）` 10 / `官方自报（T0 官方一手）` 7 / `官方论文自报（T0 直采）` 3 / `官方 GitHub 自报（T0 直采）` 1）—— 是 §27.3 前缀类的**镜像**，`confidence` 全是 `T0-自报`、与括号里的 `T0` **一致 21 / 不一致 0 / 无键 0** → 与 D18c 甲档同类，剥掉是零损失而非迁移；剥完主体仍含「自报」→ **新触发那条 WARN 0 条**；~~这类**至今无任何门禁规则照它**（6.3 只管整栏纯等级值、6.4 只管开头形）~~ **→ ✅ 规则 6.5 接管（§27.6）**。② 丙档 85 条属重采。③ 来源类型缺失 **1305**（无键 782 + null 523）。④ 该栏仍不收敛（**225 种**）。⑤ 5 条双括号观感遗留。

**文档同日改**：`prompt.md` 一处 —— 把「另有 5 条枚举外写法登记待单独处置」换成 D19 结案（五条定级结果与依据），并新增「定级不许抄兄弟条目」这条口径，指针扩到 **§27.5**；本指南 §27.4 遗留 ①② 加删除线指向本节、抬头 `temp/*.py` 轮次范围 `D2–D18e` → `D2–D19`；`archive/qa_report.md` 抬头十九→**二十处**、新增**第 20 项**、§1 与 §6.2 基线同步 118；新增 **`docs/archive/交接_2026-09-02_D19结案_换平台接手卡.md`**（面向零上下文读者的全面落盘档：现状态、命令原文、环境陷阱、待拍板清单、红线、被推翻的直觉十条）。记忆两处同步。
...
### §27.6 D20：等级值当「括号后缀」写在来源类型栏 = 21 条剥净 + 门禁新增 WARN 级规则 6.5（2026-09-02 同日续，第 28 轮拍板两问）

**第 28 轮拍板（一次两问，都按建议档）**：
① 存量 21 条怎么处置 —— **「剥等级、留限定词」**。不做「整对括号一起删」那一档：括号里除等级外还有 `直采` / `官方一手` 这类**采集者自己写的来源限定词**，删掉是有信息损失的。
② 要不要补 WARN 级门禁规则照这个形 —— **「补，并造探针」**。

**这类缺陷是什么**：`source_type` 写成 `官方模型卡自报（T0 直采）` —— 主体是真实来源描述（合规），等级值却塞在尾部括号里（不合规）。它是 §27.3（D18d）前缀类的**镜像形**，一栏背两种语义，而 6.3 只管「整栏就是一个等级值」、6.4 只管「等级值在开头」，**两条都照不到它**。

**一个反直觉的历史事实（本轮顺手量的，值得记住）**：把这条判据在现盘之外的 **26 个 `d*bak-*` 备份**上全跑一遍，命中数**从最早的 5467 条目那一份到现盘一字不变，全是 21**（`temp/d20_65_regex_check.py`）。也就是说这类写法**从建库就在，17 轮整改一次都没碰过它** —— 不是哪一轮改出来的，也不是采集期集中爆发的。前缀类 405 条能一路被清掉而它一条不动，唯一原因就是**没有任何机制指向过这个形**。**推论：一栏里同一类缺陷的「不同写法形」会被逐轮遗漏，直到有人专门去量差额；只要判据不是「任意位置」而是「以 X 开头」，就一定有镜像形活着。**

**判据选型（四个候选形 × 三个快照，先量后拍，`temp/d20_rule65_measure.py`）**

| 候选形 | 现盘 | D20 改前备份 | D18d 改前备份 |
|---|---|---|---|
| S1 括号内含等级 ∧ ¬以等级开头 | 0 | 21 | 21 |
| S2 尾部括号内含等级 ∧ ¬以等级开头 | 0 | 21 | 21 |
| **S3 任意位置含等级 ∧ ¬以等级开头（选定）** | **0** | **21** | **21** |
| S3′ = S3 ∧ ¬6.3 ∧ ¬6.4 | 0 | 21 | 21 |
| 对照：**去掉** ¬以等级开头 这道前置 | **85** | 106 | 516 |
| 对照：规则 6.3 自身命中 | 85 | 89 | 89 |

三点结论：**① 前置是必需的、不是保险** —— 去掉它，6.5 会把 6.3 在现库的 85 条命中原样重复报一遍（基线 118 → 203），这正是 D18e 躲开的那个坑。**② 括号限定的 S1/S2 与最宽的 S3 命中集完全相同**，所以选最宽那道、不必为「只盯括号」付代价；S3 还能顺带照到「等级塞在句中括号」这类本轮库里暂时没有的形状。**③ 再显式追加 `¬6.3 ∧ ¬6.4` 不改变命中集**（6.3/6.4 都要求等级值在位置 0，而 S3 已经要求「不以等级开头」），故**不写进代码**，注释里也标明别把它当第二道保险加回来。另外 `T[0-4]` 放宽成 `T[0-9]` 命中集一字不变，小数等级 `T1.5` 由 `(?:\.[0-9]+)?` 自然覆盖。

**存量处置（21 个条目 / 3 条记录，全在 `self_reported`，只改 `source_type` 一栏）**

| 记录 | 条数 | 旧写法 → 新写法 |
|---|---|---|
| `meta:llama-2-7b:base` | 10 | `官方模型卡自报（T0 直采）` → `官方模型卡自报（直采）` |
| `mistral:magistral-small-1-0:base` | 7 | `官方自报（T0 官方一手）` → `官方自报（官方一手）` |
| `deepseek:deepseekmath-v2:base` | 3+1 | `官方论文自报（T0 直采）` → `官方论文自报（直采）`；`官方 GitHub 自报（T0 直采）` → `官方 GitHub 自报（直采）` |

**零损失的依据**：这 21 条的 `confidence` 现状实测 **一致 21 / 不一致 0 / 无键 0** —— 括号里的 `T0` 与 `confidence` 栏记的是同一个值，所以剥掉的是**重复**而不是信息，与 D18c 甲档同类（不是乙档那种「只在这栏有」的迁移）。变换规则：`\s*[（(]\s*T\d…` 只删等级字面量与其后的分隔空白，**保留左右括号与限定词**。四条 rail 逐条断言：剥后 ≠ 原值、不产生空括号、`self_reported` 段剥后主体仍含「自报」（不新触发那条 WARN）、剥后不再满足 6.5 判据。

**验收（`temp/d20_measure.py` 只测 → `temp/d20_apply.py` rails 全绿才 `--apply`）**：前置三方 md5（现盘 == HEAD blob == `ceff15e9…`、门禁脚本 == D18e 那版 `6ea4de42…`）、序列化往返 933/933 逐字节、`model_id` 全库唯一、**记录 × 写法 × 条数交叉表**（4/10/7 与分段分布 `self_reported: 21`，逐字取自实测而非推测）、叶子差异 **21 处 / 越界 0**、未参与的 **930 条一字未动**、**逆还原 21/21 逐字节相等**且计数防呆写成 `assert n_rest == 21`、形状三分（非空 4337 / 无键 782 / null 523）改前==改后、含「自报」条目 **2567 → 2567**、`score` 多重集相同、`confidence` 有键 4945 → 4945，以及**「同一函数跑两个快照」的一对断言：改后复扫 0 / 改前复扫 21** —— 只有成对出现才能排除「判据恒真」和「判据恒假」两种废判据。**非空写法种类 225 → 225**（4 种旧写法消失、4 种新写法出现，且**库里原本都没有**）。

**规则 6.5 实现**：`TIER_ANYWHERE_STYPE_RE = (?:^|[^A-Za-z])T[0-4](?:\.[0-9]+)?` + 前置 `TIER_LEADING_STYPE_RE = ^\s*T[0-4](?:\.[0-9]+)?`，**跑分三段都查**。前置刻意比 6.3/6.4 的 `^T` 更宽（多容 `^\s*`），这样「带前导空格的前缀形」也不会被本条当成后缀形误报 —— 宁可漏一格排版瑕疵，也不把一类缺陷报成另一类。正则只决定要不要记 WARN，绝不驱动数据改写。

**验收（规则 6.5 侧，`temp/d20_probe.py`，10 个探针全 PASS）**：现库命中为 0，所以「这条规则真的接上线了」在真库上**拿不到任何证据** —— 写错变量名、放错循环、正则不匹配全都静默表现为「命中 0」，与「库里真干净」在门禁输出上完全无法区分。**判据本轮不再在探针脚本里抄第二份正则，而是 `import validate_model_data` 直接用门禁模块里的对象**：D18e 那版探针抄了一份 6.4 的正则且**和门禁不一样**（多写了 `\s*`）—— 探针测自己抄的那份，等于没测上线的那份。载体按三条实测条件从全库挑出 **92** 条合格、取第一条 `alibaba:qwen2-1-5b:base`（与 D18e 独立测出的是同一条，条件本身可复核）：① 整条记录**在基线报告里完全不出现**（一条 WARN/ERROR 都没有，改它不会顺手消掉别处旧命中，且「载体段非空」这道防呆才有意义）② 载体原值非空且不响 6.3/6.4/6.5 ③ `confidence` 不在 T0 家族（否则「建议体现自报属性」那条会跟着变、混淆计数）。

| 探针 | 植入值 | ERROR | WARN | 实际响 | 要验的事 |
|---|---|---|---|---|---|
| P0 | 原样重写 | 0 | 118 | — | 临时文件与真库 **md5 逐字节相同**，否则计数差里混着序列化噪音 |
| P1 | `官方模型卡自报（T0 直采）` | 0 | **119** | **仅 6.5** | 规则接通（用的就是本轮剥掉的原写法） |
| P2 | `官方自报(T2)` | 0 | **119** | **仅 6.5** | 半角括号、非 T0 等级同样照到 |
| P3 | `官方自报（T1.5 评测）` | 0 | **119** | **仅 6.5** | 小数等级由 `(?:\.[0-9]+)?` 覆盖 |
| P4 | `T0-自报` | 0 | 119 | 仅 6.3 | 与 6.3 **互斥** |
| P5 | `T0 官方自报` | 0 | 119 | 仅 6.4 | 与 6.4 **互斥** |
| P6 | `" T0 官方自报"`（前导空格） | 0 | 118 | 无 | 宽前置生效：不被误报成后缀形（**已知取舍**：这一格形三条规则都不响） |
| P7 | `官方自报（XT0 批次）` | 0 | 118 | 无 | 左边界 `[^A-Za-z]` 生效：字母粘连不误伤 |
| P8 | `官方模型卡自报（直采）` | 0 | 118 | 无 | **D20 剥后的正确写法本身不误伤** |
| P9 | `官方技术报告` | 0 | 118 | 无 | 无等级的正常写法不误伤 |

**负对照**：新门禁 × `…d20bak-20260902-231130`（D19 版，那 21 条还在）= **139 = 118 + 21**，且与现盘报告的差异剥离标题/摘要后是 **27 增 0 删**，其中 21 条正是 6.5 的消息、其余 **6 行是那 3 条记录的 `## model_id` 分段标题 + 空行**（它们在现盘一条 WARN 都没有，所以整个分段在报告里不存在）。**这一步差点被误读成「多出 6 条判据外的命中」** —— 门禁报告的分段标题本身是内容的一部分，比对差异时要先认清哪些行是结构、哪些行是命中。

**本轮教训（同类第六次）**：提请拍板用的那张表里，`111 / 90` 是 **D19 改前快照**的口径，现盘是 `106 / 85` —— **沿用自己上一轮记录的数字当本轮动手依据是错的，必须先重测**。重测同时暴露了另一件事：中口径与「以等级开头」的差额上一轮是 21、这一轮也是 21，**数值相同不代表同一盘**，只有重新量才知道。另一条操作性细节：`entries()` 这类遍历辅助函数被单条记录和记录列表两种调用点共用时，要显式兼容两种入参，别靠调用点纪律（本轮栽了两次 `AttributeError: 'list' object has no attribute 'get'`）。

**仍开着的遗留**：① ~~这类括号后缀形**至今无规则照它**~~ **✅ 本轮结案（数据 21 条 + 规则 6.5）**；② 丙档 **85 条**两栏矛盾属重采（也是 6.3 升 ERROR 前必须清掉的那批）；③ 来源类型缺失 **1305**（无键 782 + null 523，两种形状）门禁放过；④ 该栏**仍不收敛**（225 种写法，本轮 4 进 4 出、净零）；⑤ 5 条双括号观感遗留；⑥ **P6 暴露的新盲区**：`" T0 官方自报"` 这种带前导空格的前缀形三条规则都不响 —— 现库实测 **0 条**，属纯理论缺口，登记不动门禁（刻意保留宽前置换来的「不误报另一类」，比多照一格排版瑕疵值）。

**文档同日改**：本指南新增 **§27.6**、§27.5 遗留 ① 加删除线指向本节、抬头 `temp/*.py` 轮次范围 `D2–D19` → `D2–D20`、版本留痕新增 **v3.6**；`prompt.md` 一处 —— 把 D19 写下的「6.3 与 6.4 都照不到这 21 条」前瞻注换成 6.5 实况，指针扩到 **§27.6**；`archive/qa_report.md` 抬头二十→**二十一处**、新增**第 21 项**、§1 与 §6.2 基线同步；`docs/archive/交接_2026-09-02_D19结案_换平台接手卡.md` 更新基线数字与 md5 链、§5 待拍板第 ① 项结案、§4 改写为 D20。记忆两处同步。

**新基线：933 条 / ERROR 0 / WARN 118（数值不变：常量段 16 + 规则 6.2 的 17 + 规则 6.3 的 85 + 规则 6.4 的 0 + 规则 6.5 的 0）/ 结构漂移 0 / 精确主键重复 17 组；数据 md5 `ceff15e9a12709ab3d49679f3d1560f5` → `be9985e1d54a20b3734d2746766e742c`（8161763 字节），门禁脚本 md5 `6ea4de427c0875375b9fd83f6f0cf1c3` → `4ab840522d03156c31a0c90c4eb2b783`，备份 `model_data_v2.jsonl.d20bak-20260902-231130`（= D19 版）。**

**md5 三向判别又添一例**：本轮**数据步**（剥 21 条）数据 md5 变了而两份门禁报告除标题外**逐字节相同**（WARN 都是 118）—— 门禁对这个形完全看不见，改动价值纯语义，与 D18d 同型；本轮**门禁步**（加 6.5）门禁 md5 变了而真库 WARN 仍是 118 —— 与 D18e 同型。同一天里两个「WARN 不变」挨着发生，靠哪个 md5 动了才能分辨。
