...
> **⚠ 文中 `temp/*.py` 指针不入库**：`.gitignore` 第 6 行排除了 `/temp/`，那些脚本是 D2–D13 各轮整改的**本机一次性产物**，
...
> 4. 门禁 `WARN` 现值应为 **474**（= 规则 6.2 的 17 + 其余 457；D13 废止两条「上下文须独立实测」类 WARN 前是 706）。
...
689 这个基线只统计主库，隔离档不计入（主库基线 **D11 起 703、D12 起 706、D13 起 474**，见 §20、§21、§22）；但**回流主库前必须先过归一脚本**，否则等于把 D9/D10 已结案的问题重新引进来。
...
**现基线：940 条 / ERROR 0 / WARN 474（= 457 + 规则 6.2 的 17）/ 结构漂移 0 / 精确主键重复 17 组。**
