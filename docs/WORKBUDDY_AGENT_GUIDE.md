...
- 其余登记 D18：yandex 两条 notes 三栏整段复制；**436 条 `self_reported` 的 `source_type` 不含「自报」**（门禁 WARN，抄写源是文档下拉表，见 §26.5 第 3 项）；`docs/交接_2026-09-01_D16结案.md` 把远端仓库写成「GitHub 私有」，实为**公共**仓库（已改正）——写交接文档时对「远端可见性」这类安全相关事实必须实测，不能凭印象。
