...
> **⚠ 文中 `temp/*.py` 指针不入库**：`.gitignore` 第 6 行排除了 `/temp/`，那些脚本是 D2–D18b 各轮整改的**本机一次性产物**，
...
**升 ERROR 的条件**（沿用 §24 规则 1.1/1.2 的写法，避免下一轮凭感觉升级）：下一个采集周期**新记录命中 0**，且这 607 条已重采补全来源类型或按拍板处置完毕。在此之前它必须停在 WARN —— 现库存量 607 条，判 ERROR 会让门禁对全库直接失败，历史验收信号全部失真。
...
**现基线：933 条 / ERROR 0 / WARN 640（= 常量段 16 + 规则 6.2 的 17 + 规则 6.3 的 607）/ 结构漂移 0 / 精确主键重复 17 组。** 数据 md5 与 D18 收尾时**完全相同**（本轮零数据改动），凡以 WARN 33 为基线的表述自此过时。
