...
> **⚠ 文中 `temp/*.py` 指针不入库**：`.gitignore` 第 6 行排除了 `/temp/`，那些脚本是 D2–D18e 各轮整改的**本机一次性产物**，
...
**仍开着的遗留**：① 表外 `T1.5-第三方评测` **1 条**（6.4 刻意照不到，属「枚举外可信度值」那一项，与 §27.2 表外 4 条同类）；② §27.2 表外 **4 条**（`T2-第三方` 3 + `T3-转述` 1，等级本身有争议）；③ 丙档 **85 条**属重采；④ 来源类型缺失 **1300**（三段，两种形状）门禁放过、只能靠清单追踪；⑤ 该栏**仍不收敛**（228 种写法）；⑥ 5 条双括号观感遗留。
