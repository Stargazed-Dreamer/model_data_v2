...
## 3. 填充率对比（花名册 702 vs v1 遗留 248）

统计脚本：`scripts/qa_stats.py`；填充判定 = 非 null / 非空串 / 非空数组 / 非全 null 对象。

| 维度 | 花名册(702) | v1 遗留(248) | 差值 |
|---|---:|---:|---:|
| 发布日期 | 100.0% | 100.0% | 0.0 |
| **定位标签** | **89.5%** | 81.9% | **+7.6** |
| 开放权重 | 94.7% | 87.1% | +7.6 |
| API 可用 | 92.2% | 85.5% | +6.7 |
| 本地部署 | 88.3% | 75.4% | +12.9 |
| **总参数量** | **74.5%** | 56.0% | **+18.5** |
| 激活参数量 | 49.3% | 48.0% | +1.3 |
| **上下文窗口** | **67.0%** | 97.2% | −30.2 |
| 有效上下文 | 17.0% | 35.9% | −18.9 |
| 知识截止 | 16.4% | 25.4% | −9.0 |
| **多模态输入** | **82.6%** | 94.4% | −11.8 |
| 原生多模态 | 79.8% | 89.1% | −9.3 |
| **自报跑分** | **56.0%** | 66.1% | −10.1 |
| 独立评测 | 18.2% | 49.2% | −31.0 |
| Arena Elo | 6.4% | 47.6% | −41.2 |
| 定价-输入 | 22.1% | 59.3% | −37.2 |
| 定价-输出 | 20.2% | 58.5% | −38.3 |
...
2. **定价必须按分型看**。红线 5 要求开源权重模型 `pricing` 全 null（官方不公布 API 调用价），
   混在一起统计会严重低估真实覆盖：
   ...
   | v1 遗留 / 开源权重（131 条） | 32.8%（**偏高，疑似混入了第三方云转售价**） |

   → 真实的同比口径是 **50.3% vs 86.7%**（同为商业 API 模型），差距仍在，
   主因是花名册里的商业 API 模型有大量长尾厂商（如区域小厂），定价页难找或不存在。
   → 反过来，v1 遗留的开源权重模型有 32.8% 填了定价，反而**违反红线 5**，建议后续核查。
...
6. **v1 遗留记录的开源权重定价可疑**。131 条 v1 遗留的开源权重模型中有 43 条（32.8%）填了定价，
   与红线 5（开源权重模型 pricing 应为 null）冲突，疑似使用了第三方云厂商转售价，**建议核查**。
...
| 项 | 说明 | 建议 |
|---|---|---|
| WARN 678 | 主要是上下文窗口 notes 规范（228 条）与自报分 source_type（~220 条） | notes 补写轮，属采集性质工作 |
| v1 遗留开源权重定价 | 131 条中 43 条有定价，疑似违规 | 核查后按红线 5 置 null |
