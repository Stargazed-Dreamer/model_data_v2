...
> **【同日补记 2026-08-29 起 · 整改轮 D1–D18】** 本报告的质检结论之后，同日又跑了一轮整改，读本文时注意十五处时效性：
...
>    **→ 续：D15 / D16 / D17c 三轮基线均为 469 未变，D18 起降为 33（= 常量段 16 + 规则 6.2 主键撞车 17），现库 ERROR 0 / WARN 33，见第 15 项**
...
>    ④ **门禁判据本身有个洞没堵** —— 那条 WARN 是**纯子串匹配** `"自报" in source_type`，于是把可信度值当来源类型写
>    （`"source_type": "T0-自报"`）天然满足它、能过门禁却**零来源信息**，实测跑分三段共 **607 条 / 62 条记录**，
>    另有 405 条把 `T0` 前缀缀在真实来源描述前。**含义：这条 WARN 归零有一部分是空心的** —— 归零只说明「不再有裸写法」，
>    不等于「来源类型栏都写了来源类型」；升成语义判断要维护合法来源类型白名单（现库 191 种写法），成本未评估 → 存量待拍板，
>    新增已在 `prompt.md` 用显式禁令挡住。机制与全部教训见指南 **§27**。
>    **⚠️ 本报告正文里凡以 WARN 469 为基线的表述，自 D18 起过时**（现基线 **33 = 常量段 16 + 主键撞车 17**）；
...
> **整改后现值（读表时替换使用）**：主库 **933 条**（花名册 685 + v1 遗留 248）、ERROR **0**、~~WARN **469**~~ **WARN 33**
...
> 现值 **33 = 常量段 16 + 规则 6.2 主键撞车 17**）、
> 结构漂移 0；另有 10 条存疑记录原样存 `docs/unconfirmed_models.jsonl`、7 条非模型记录原样存
> `docs/non_model_records.jsonl`。见文首补记第 4、8、9、10、11、15 点。
...
| 项 | 说明 | 建议 |
|---|---|---|
| WARN 678 → **689**（不含规则 6.2 的固定基线）→ 706 → 474 → ~~现值 **469**~~ → **33**（D18）| 689 的实测构成：自报分 `source_type` 未含「自报」440、有效上下文缺测试方法说明 127、标称上下文缺「待测」标注 105、`knowledge_cutoff` 格式 9、参数量缺未披露声明 4、`source_type` 与价格值矛盾 3、缺 `source_url` 1。**这 689 条从 D9 到 D12 全程一字未动**，用作负对照的常量段；D11 加规则 6.2（14 组）→ 703、D12 拆来源后 17 组 → 706、**D13 废止其中两条上下文 WARN（127 + 105）后常量段降到 457** = 474、**D14 移出 7 条非模型再减 5（自报分 4 + 参数量声明 1）→ 常量段 452**，加规则 6.2 的 17 组 = **469**；**D15 / D16 / D17c 三轮 469 未变**；**D18 把「自报分 `source_type` 缺『自报』」这一条 WARN 拆成两类缺陷后清存量 —— 门禁改为放过空值 → 341（−128，恰为空值条数），数据补 308 条「自报」→ 33（−308，恰为非空缺「自报」条数），常量段 452 → 16** | ~~剩余最大项是 436 条自报分 `source_type`，属 notes 补写轮性质的采集工作~~ **→ D18 已结案：308 条挂后缀归一、128 条空值由门禁放过并登记待重采，见第 15 项**；其余为新增检查照出的存量（`knowledge_cutoff` 格式 9、参数量缺未披露声明 3、`pricing.source_type` 与价格值矛盾 3、有定价缺 `source_url` 1）。规则 6.2 那 17 条不是脏数据，是待复测清单本身。**注意 D18 也留下一条门禁盲区：那条 WARN 是纯子串匹配，`"source_type": "T0-自报"` 这种把可信度值写进来源类型栏的也能过（实测 607 条），所以归零是「不再有裸写法」而非「都写了来源类型」** |
