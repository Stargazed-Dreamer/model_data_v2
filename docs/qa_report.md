...
> **【同日补记 2026-08-29 起 · 整改轮 D1–D16】** 本报告的质检结论之后，同日又跑了一轮整改，读本文时注意十三处时效性：
...
>    **`available` 的真实语义是「存在某条免费途径」而不是「这个模型免费」**，因此它**不得参与也不得抵消价格判断**——
...
>    键序 215 条全一致、`available` 分布 `True 120 / False 36 / null 59`，反向还原逐字节复原、备份 md5 与改前 HEAD 相同。
>    门禁新增规则 **4.4**（非对象形状即 WARN），**负对照用改前备份实测命中 81**（550 = 469 + 81），改后命中 0。
>    四份采集文档同日补齐，否则下批采集会照旧写回字符串。机制与判据见指南 **§25**。
>    **遗留两条**：① 5 条 `available=null` 未纳入扫描，其中 `mistral:magistral-small-1-0`「模型已从官方 API 退役」
>    按分叉 C 应为 `false`，因 notes 未命中扫描判据而漏网，是唯一一条真漏；② **一处口径张力待用户定夺** ——
