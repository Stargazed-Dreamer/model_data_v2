...
>
> **⚠ 文中 `temp/*.py` 指针不入库**：`.gitignore` 排除了 `/temp/`，那些是 D2–D13 各轮整改的本机一次性脚本，
> 克隆仓库看不到它们（拍板于 2026-08-30，与 `WORKBUDDY_AGENT_GUIDE.md` 开头同一声明）。
> 判据、负对照数字与验收结论都写在正文里，复核请按这些做；常驻工具只有 `scripts/` 下那几个。

> **【同日补记 2026-08-29 起 · 整改轮 D1–D13】** 本报告的质检结论之后，同日又跑了一轮整改，读本文时注意十处时效性：
...
> **整改后现值（读表时替换使用）**：主库 **940 条**（花名册 692 + v1 遗留 248）、ERROR **0**、WARN **474**
...
> 再把 27 条两栏填反的记录归位 → **474**）、
> 结构漂移 0；另有 10 条存疑记录移出主库、原样存 `docs/unconfirmed_models.jsonl`。见文首补记第 4、8、9、10 点。
...
| 项 | 说明 | 建议 |
|---|---|---|
| WARN 678 → **689**（不含规则 6.2 的固定基线）→ 706 → 现值 **474** | 689 的实测构成：自报分 `source_type` 未含「自报」440、有效上下文缺测试方法说明 127、标称上下文缺「待测」标注 105、`knowledge_cutoff` 格式 9、参数量缺未披露声明 4、`source_type` 与价格值矛盾 3、缺 `source_url` 1。**这 689 条从 D9 到 D12 全程一字未动**，用作负对照的常量段；D11 加规则 6.2（14 组）→ 703、D12 拆来源后 17 组 → 706、**D13 废止其中两条上下文 WARN（127 + 105）后常量段降到 457**，加规则 6.2 的 17 组 = **474** | 剩余最大项是 440 条自报分 `source_type`，属 notes 补写轮性质的采集工作；其余为新增检查照出的存量。规则 6.2 那 17 条不是脏数据，是待复测清单本身 |
