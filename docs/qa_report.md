...
> **【同日补记 2026-08-29 起 · 整改轮 D1–D18b】** 本报告的质检结论之后，同日又跑了一轮整改，读本文时注意十六处时效性：
...
>    **⚠️ 本报告正文里凡以 WARN 469 为基线的表述，自 D18 起过时**（~~现基线 **33 = 常量段 16 + 主键撞车 17**~~ **D18b 起为 640 = 常量段 16 + 主键撞车 17 + 规则 6.3 的 607**）；
...
> **整改后现值（读表时替换使用）**：主库 **933 条**（花名册 685 + v1 遗留 248）、ERROR **0**、~~WARN **469**~~ ~~**WARN 33**~~ **WARN 640**
...
> `docs/non_model_records.jsonl`。见文首补记第 4、8、9、10、11、15、16 点。
...
| 项 | 说明 | 建议 |
|---|---|---|
| WARN 678 → **689**（不含规则 6.2 的固定基线）→ 706 → 474 → ~~现值 **469**~~ → **33**（D18）→ **640**（D18b 只加规则 6.3）| 689 的实测构成：自报分 `source_type` 未含「自报」440、有效上下文缺测试方法说明 127、标称上下文缺「待测」标注 105、`knowledge_cutoff` 格式 9、参数量缺未披露声明 4、`source_type` 与价格值矛盾 3、缺 `source_url` 1。**这 689 条从 D9 到 D12 全程一字未动**，用作负对照的常量段；D11 加规则 6.2（14 组）→ 703、D12 拆来源后 17 组 → 706、**D13 废止其中两条上下文 WARN（127 + 105）后常量段降到 457** = 474、**D14 移出 7 条非模型再减 5（自报分 4 + 参数量声明 1）→ 常量段 452**，加规则 6.2 的 17 组 = **469**；**D15 / D16 / D17c 三轮 469 未变**；**D18 把「自报分 `source_type` 缺『自报』」这一条 WARN 拆成两类缺陷后清存量 —— 门禁改为放过空值 → 341（−128，恰为空值条数），数据补 308 条「自报」→ 33（−308，恰为非空缺「自报」条数），常量段 452 → 16**；**D18b 只加门禁留痕、数据一字未动：规则 6.3 命中 607 → 33 + 607 = 640，数据 md5 `b5eaf743…` 未变** | ~~剩余最大项是 436 条自报分 `source_type`，属 notes 补写轮性质的采集工作~~ **→ D18 已结案：308 条挂后缀归一、128 条空值由门禁放过并登记待重采，见第 15 项**；其余为新增检查照出的存量（`knowledge_cutoff` 格式 9、参数量缺未披露声明 3、`pricing.source_type` 与价格值矛盾 3、有定价缺 `source_url` 1）。规则 6.2 那 17 条不是脏数据，是待复测清单本身。**注意 D18 也留下一条门禁盲区：那条 WARN 是纯子串匹配，`"source_type": "T0-自报"` 这种把可信度值写进来源类型栏的也能过（实测 607 条），所以归零是「不再有裸写法」而非「都写了来源类型」** **→ D18b 已加规则 6.3 把这 607 条照出来（WARN 级留痕、数据一字不动），见第 16 项** |
