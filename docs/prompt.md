...
- 来源类型下拉：官方技术报告 / **官方技术报告（自报）** / 官方定价页 / Model Card / **官方 Model Card（自报）** / 独立评测平台 / Hugging Face / 厂商博客 / 行业媒体 / 行业媒体转述官方发布 / **行业媒体聚合官方发布（自报分转述）** / 社交媒体。  
  ...
  >
  > **种类数会随归一轮下降，但「含自报的条目数」不该动**：D18d 把种类从 178 收到 171，而条目数 **2567 → 2567 一条不变** —— 这一栏的整改只该合并重复写法，不该给任何条目加上或摘掉「自报」属性。验收时就按这两个数一起看。（**D19 后复测仍为 171 / 2567**：那轮改的 5 条写法都不含「自报」，两个量本就该不动。）
  > **注意「种类数」有两个口径，别混用**：**含「自报」的写法 171 种**（本节这两个数）vs **三段全扫的非空写法 225 种**（D18d 后 228，D19 置空 5 条后 225）。前者是「自报归一」这一类的验收量，后者是整栏收敛度的观测值。
  >
  ...
  > 等级请写进 `confidence`，来源类型请写「是什么文件/页面」。机制与全部验收数字见 `WORKBUDDY_AGENT_GUIDE.md` **§27.1 / §27.2 / §27.3 / §27.4 / §27.5**。
