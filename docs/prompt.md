...
- 来源类型下拉：官方技术报告 / **官方技术报告（自报）** / 官方定价页 / Model Card / **官方 Model Card（自报）** / 独立评测平台 / Hugging Face / 厂商博客 / 行业媒体 / 行业媒体转述官方发布 / **行业媒体聚合官方发布（自报分转述）** / 社交媒体。  
  ...
  >
  > **⚠️ 别把可信度值当来源类型写**：`"source_type": "T0-自报"` 这种也含「自报」二字，**能过门禁但零来源信息**（门禁只查子串，是已知盲区）。库里现存 **607 条**这类纯等级值（`T0-自报` 379 / `T0-自报-转述` 200 / `T0` 22 / `T2-第三方` 3 / `T3` 2 / `T3-转述` 1，涉及 62 条记录），另有 405 条把 `T0` 前缀缀在真实来源描述前（信息不丢但前缀冗余）。等级请写进 `confidence`，来源类型请写「是什么文件/页面」。
