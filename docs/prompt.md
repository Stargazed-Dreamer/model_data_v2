...
- 来源类型下拉：官方技术报告 / **官方技术报告（自报）** / 官方定价页 / Model Card / **官方 Model Card（自报）** / 独立评测平台 / Hugging Face / 厂商博客 / 行业媒体 / 行业媒体转述官方发布 / **行业媒体聚合官方发布（自报分转述）** / 社交媒体。  
  ...
  > **新采集写出纯等级值会立刻吃一条 WARN**。库里现存 **607 条**这类值（`T0-自报` 379 / `T0-自报-转述` 200 / `T0` 22 / `T2-第三方` 3 / `T3` 2 / `T3-转述` 1，涉及 62 条记录），
  > 经拍板**数据一字未改**、原样在册由门禁自动维护清单（md5 与 D18 收尾时相同）。另有 **406 条**把 `T0` 前缀缀在真实来源描述前
  > （如 `T0 官方一手技术报告（自报）`，信息不丢但前缀冗余，**规则 6.3 刻意不报这类** —— 一条 WARN 只背一类缺陷）。
  > 等级请写进 `confidence`，来源类型请写「是什么文件/页面」。机制与全部验收数字见 `WORKBUDDY_AGENT_GUIDE.md` **§27.1**。
