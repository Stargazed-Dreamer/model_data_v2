...
- 开源权重模型如果没有官方托管 API 价：在 `pricing.notes` 中标注“本地部署成本”或“社区托管价”，不与其他官方 API 价直接比较。  
...
- `currency`：默认 `"USD"`；非 USD 价格需换算并在 `notes` 说明。  
- `unit`：默认 `"per_million_tokens"`。  
