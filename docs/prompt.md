...
- 来源类型下拉：官方技术报告 / **官方技术报告（自报）** / 官方定价页 / Model Card / **官方 Model Card（自报）** / 独立评测平台 / Hugging Face / 厂商博客 / 行业媒体 / 行业媒体转述官方发布 / **行业媒体聚合官方发布（自报分转述）** / 社交媒体。  
  > **门禁的真实判据只有一条：`self_reported` 条目的 `source_type` 必须含「自报」二字**（WARN 级，且仅在 `confidence` 属 T0 / T0-自报 / T0-自报-转述 时检查）。上面下拉表里带「自报」的三项是**推荐取值**，不是穷举 —— 库里现有 ~~178~~ **171** 种含「自报」的写法（**D18d 后**），按「原类型 +（自报）」自行组合也合规（如 `官方发布公告（自报）`）；以 `（转述）` 结尾的请写成 `（自报分转述）`，不要叠成 `（转述）（自报）`。不带「自报」的同名类型留给 `independent` 引用官方报告时使用。
  >
  > **种类数会随归一轮下降，但「含自报的条目数」不该动**：D18d 把种类从 178 收到 171，而条目数 **2567 → 2567 一条不变** —— 这一栏的整改只该合并重复写法，不该给任何条目加上或摘掉「自报」属性。验收时就按这两个数一起看。（**D19 后复测仍为 171 / 2567**：那轮改的 5 条写法都不含「自报」，两个量本就该不动。**D20 后复测仍为 171 / 2567**：那轮剥掉 21 个条目括号里的等级后缀，4 种旧写法消失的同时 4 种新写法出现，故两个量都不动。若某一轮做完这两个数有一个动了，说明整改越界了。）
  > **注意「种类数」有两个口径，别混用**：**含「自报」的写法 171 种**（本节这两个数）vs **三段全扫的非空写法 225 种**（D18d 后 228，D19 置空 5 条后 225，**D20 后仍 225**）。前者是「自报归一」这一类的验收量，后者是整栏收敛度的观测值。
  >
  > 存量已于 **D18（2026-09-02）结案**：436 条踩 WARN 的裸写法里 **308 条**已挂后缀归一、**128 条**是 `source_type` 为空（门禁改为不再报空值，登记为待重采）。新采集不得再写裸类型。
  >
  > **⚠️ 别把可信度值当来源类型写**：`"source_type": "T0-自报"` 这种含「自报」二字，**能过上面那条「必须体现自报」的子串判据，但零来源信息**。
  > **D18b（2026-09-02）起门禁另有规则 6.3（WARN）专门照这类**：`source_type` **整栏就是一个可信度等级值**即报，跑分三段（`self_reported` / `independent` / `arena_elo`）都查，
  > **新采集写出纯等级值会立刻吃一条 WARN**。库里现存 ~~607~~ ~~**89**~~ **85** 条这类值（~~`T0-自报` 379 / `T0-自报-转述` 200 / `T0` 22 / `T2-第三方` 3 / `T3` 2 / `T3-转述` 1，涉及 62 条记录~~
  > **D18c（2026-09-02）整改后：`T0-自报` 84 / `T2-第三方` 3 / `T0-自报-转述` 1 / `T3-转述` 1，涉及 12 条记录**），
  > ~~经拍板**数据一字未改**、原样在册由门禁自动维护清单（md5 与 D18 收尾时相同）~~
  > **D18c 已按第 24 轮拍板处置 518 条**：乙档 **395** 条把等级值**逐字迁进** `confidence`（该栏原本连键都没有）+ 甲档 **123** 条清掉与 `confidence` 同值的冗余，
  > 两档的 `source_type` 一并置 `null`（键保留、不删）。剩 **89** 条 = 丙档 **85**（两栏矛盾，属重采）+ ~~枚举外 **4**（等级本身有争议，原样不动、单独登记）~~ **✅ D19 已结案**。现库 **85** 条 = 丙档全部（见下）。
  > ~~另有 **406 条**把 `T0` 前缀缀在真实来源描述前~~
  > ~~（如 `T0 官方一手技术报告（自报）`，信息不丢但前缀冗余，**规则 6.3 刻意不报这类** —— 一条 WARN 只背一类缺陷）。~~
  > **D18d（2026-09-02）已按第 25 轮拍板结案**：那 406 条里 **405** 条已剥掉等级前缀（正文一个字没动），其中 **55** 条的「自报」二字原先只活在前缀里（如 `T0-自报 官方一手`），按上面同一条规矩补挂成 `官方一手（自报）`；
  > **5** 条等级与描述**粘连无分隔符**的（`T0-自报-技术报告`，可信度栏原本连键都没有）复用 D18c 的变换把 `T0-自报` 逐字迁进 `confidence`、来源类型留 `技术报告（自报）`；
  > **1** 条 `T1.5-第三方评测`（`deepseek:deepseekmoe-16b:base`）**原样不动** —— `T1.5` 不在 `CONFIDENCE_ENUM` 内，迁进去即 ERROR，改写成 `T1`/`T2` 又等于替采集者做判断，已单独登记待重采。
  > **✅ 该条已由 D19（第 27 轮）按记录内证据定级结案**：媒体报道第三方评测属**二手转述**，按 §5.1 距离原则记 **`T3`**、来源类型栏置 `null` 待重采 —— `T1.5` 这个等级从来就不存在，是采集者自造的中间档。
  > 剥掉的等级信息**零损失**：逐条核过，被剥的前缀在改后 `confidence` 里都读得到（同值 89 条 / `confidence` 更具体 316 条，如前缀 `T0` 而可信度栏是 `T0-自报`）。
  > **⚠️ 新采集不得把等级值当前缀写进 `source_type`**（如 `T0 官方自报`）：等级只写 `confidence`，来源类型只写「是什么文件/页面」。
  > ~~**目前没有任何门禁规则照这类**（规则 6.3 只管「整栏就是一个等级值」），所以这条禁令**只有本文档在守** —— 而文档自己错过两代（见 §26.5），别指望它会替你兜住。~~
  > **D18e（2026-09-02）已按第 26 轮拍板补上门禁留痕 = WARN 级规则 6.4**：`source_type` 以 `T0`–`T4` 等级值开头、后面还有分隔符和真实描述即报。
  > 这条禁令**不再只有本文档在守**了 —— 新采集写 `T0 官方自报` 会立刻吃一条 WARN。**现库命中为 0**（存量 405 条已被 D18d 剥净），所以它纯粹是防回归，WARN 基线仍是 **122**（D19 后为 **118**，见下）。
  > 与规则 6.3 天然互斥（6.3 查「整栏就是一个等级值」、6.4 查「等级值只是前缀」），一条 WARN 只背一类缺陷。
  > ~~**它照不到上面那 1 条 `T1.5-第三方评测`**（`T1.5` 后接的是 `.` 不是分隔符），这是**有意**的 —— 那条的缺陷是「等级值不在枚举内」而不是「前缀冗余」，仍只由本文档与 §27.3 清单追踪。~~
  > **✅ D19 已结案**：那 1 条连同 §27.2 的 4 条，共 **5 条枚举外写法**已全部按记录内证据逐条定级完毕，`source_type` 栏现已 **0 条枚举外等级写法**。
  > ~~**⚠️ 但「`source_type` 栏任意位置出现等级形状」仍有 21 条**（如 `官方模型卡自报（T0 直采）`，等级写在**括号后缀**里）—— 这类**规则 6.3 与 6.4 都照不到**（6.3 只管整栏纯等级值、6.4 只管开头形），新采集同样不要这么写。~~
  > **✅ D20（2026-09-02，第 28 轮）已结案**：那 **21 条**按拍板 **「剥等级、留限定词」** 改完（`官方模型卡自报（T0 直采）` → `官方模型卡自报（直采）`）——
  > 等级本来就在同一条目的 `confidence` 里且与括号里的值**逐条一致（21/21）**，所以剥掉零损失；**限定词「直采」「官方一手」是采集者自己写的来源属性说明，必须留着，不能整对括号删掉**。
  > 同时门禁补上 WARN 级**规则 6.5** 照这个后缀形（6.4 的镜像）：判据 = 非空 ∧ **任意位置**出现等级形状 ∧ **不以等级值开头**。
  > **后一道前置（不以等级值开头）不可省**：去掉它，规则 6.3 现库命中的 85 条会被 6.5 原样重复报一遍（118 → 203），一条 WARN 就背了两类缺陷。
  > 存量已剥净 → **现库命中 0**，纯粹是防新采集回归；**新采集把等级写进括号同样会立刻吃一条 WARN**。
  > 至此该栏「等级值混进来」的三种形状（整栏纯等级值 6.3 / 前缀 6.4 / 后缀 6.5）都有规则照，且三者互斥。
  > 等级请写进 `confidence`，来源类型请写「是什么文件/页面」。机制与全部验收数字见 `WORKBUDDY_AGENT_GUIDE.md` **§27.1 / §27.2 / §27.3 / §27.4 / §27.5 / §27.6**。
- 可信度等级：T0 / T0-自报 / T0-自报-转述 / T1 / T2 / T3 / T4（新增 `T0-自报-转述`，见决策 2）。  
  > **⚠️ 这七个值是穷举，不得自造写法**：门禁按 `CONFIDENCE_ENUM` 成员判定，`confidence` 写别的**直接是 ERROR**（不是 WARN），会让整批数据过不了门禁。
  > ~~库里现存 **4 条**枚举外写法（`T2-第三方` 3 / `T3-转述` 1），是 D18c 唯一没能机械处置的部分~~
  > **✅ D19（第 27 轮）已全部结案**：连同 §27.3 那 1 条 `T1.5-第三方评测` 共 **5 条**，按**本条记录自己的**证据逐条定级进 `confidence`、`source_type` 置 `null` 待重采。
  > **`source_type` 栏现在 0 条枚举外等级写法**（三档判据各扫过一遍；递归扫全字段另有 2 条命中，但都在 `pricing.notes` / `meta.notes` 的自由文本里，不属本栏）。
  > 它们原先写在 `source_type` 栏（当时只吃一条 WARN），逐字迁进 `confidence` 会立刻把 WARN 换成 **ERROR**；机械改写成 `T2` / `T3` 又等于替采集者做判断 —— 这 4 条的等级**本身就有争议**：
  > `cognition:swe-1-6:base` 的 `notes` 明写「转述自 Cognition 官方博客」，按决策 2 该记 `T0-自报-转述`，记 `T3-转述` 等于把**厂商的话**当成**媒体的话**；
  > `mistral:mistral-large-3:base` 独立评测段 `[5][6][7]` 三条同址（网址均为 OpenRouter）同标 `T2-第三方`，其中 `[5]` 的 `notes` 明写「Artificial Analysis 评测」，
  > 而 §5.1 把 AA 列为 **T1** 的典型例子 —— 该记 T1 还是「T1 经第三方托管转述」机械推不出来。
  > **定级结果**（第 27 轮逐条裁定）：`T3-转述` → **`T0-自报-转述`**（决策 2 的强制口径）；`[5]` → **`T1`**（本条 notes 有 AA 证据）；`[6][7]` → **`T2`**（本条无归属证据）；`T1.5-第三方评测` → **`T3`**。
  > **⚠️ 定级不得抄兄弟条目**：同一记录里 `[5]` 有 AA 证据而 `[6][7]` 没有，就必须是 `T1` / `T2` / `T2` 三个不同结果。
  > 三条同址同标看着像该同判，但**拿别人的证据给自己的条目背书**就是把推断当成了来源；每条只认**自己 `notes` / `source_url` 里写下的**证据。
  > **正确做法**：「第三方」「转述」「经托管」这类限定写进 `source_type` 或 `notes`，不要缀在等级后面。
