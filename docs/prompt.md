...
- 来源类型下拉：官方技术报告 / **官方技术报告（自报）** / 官方定价页 / Model Card / **官方 Model Card（自报）** / 独立评测平台 / Hugging Face / 厂商博客 / 行业媒体 / 行业媒体转述官方发布 / **行业媒体聚合官方发布（自报分转述）** / 社交媒体。  
  ...
  >
  > **种类数会随归一轮下降，但「含自报的条目数」不该动**：D18d 把种类从 178 收到 171，而条目数 **2567 → 2567 一条不变** —— 这一栏的整改只该合并重复写法，不该给任何条目加上或摘掉「自报」属性。验收时就按这两个数一起看。
  >
  ...
  > **新采集写出纯等级值会立刻吃一条 WARN**。库里现存 ~~607~~ **89** 条这类值（~~`T0-自报` 379 / `T0-自报-转述` 200 / `T0` 22 / `T2-第三方` 3 / `T3` 2 / `T3-转述` 1，涉及 62 条记录~~
  ...
  > 两档的 `source_type` 一并置 `null`（键保留、不删）。剩 **89** 条 = 丙档 **85**（两栏矛盾，属重采）+ 枚举外 **4**（等级本身有争议，原样不动、单独登记）。
  ...
  > 这条禁令**不再只有本文档在守**了 —— 新采集写 `T0 官方自报` 会立刻吃一条 WARN。**现库命中为 0**（存量 405 条已被 D18d 剥净），所以它纯粹是防回归，WARN 基线仍是 **122**。
  ...
  > 等级请写进 `confidence`，来源类型请写「是什么文件/页面」。机制与全部验收数字见 `WORKBUDDY_AGENT_GUIDE.md` **§27.1 / §27.2 / §27.3 / §27.4**。
- 可信度等级：T0 / T0-自报 / T0-自报-转述 / T1 / T2 / T3 / T4（新增 `T0-自报-转述`，见决策 2）。  
  ...
  > 库里现存 **4 条**枚举外写法（`T2-第三方` 3 / `T3-转述` 1），是 D18c 唯一没能机械处置的部分：它们原先写在 `source_type` 栏（当时只吃一条 WARN），
  > 逐字迁进 `confidence` 会立刻把 WARN 换成 **ERROR**；机械改写成 `T2` / `T3` 又等于替采集者做判断 —— 这 4 条的等级**本身就有争议**：
